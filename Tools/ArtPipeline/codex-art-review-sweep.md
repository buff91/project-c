...
2. 처리할 각 피드백마다 분석을 시작하기 직전에
   `python3 Tools/ArtPipeline/art_runner.py feedback-progress ID "지금 확인하는 내용"`을 실행한다.
   이미 `processing`이면 중복 알림 없이 계속 처리한다.
3. 각 pending/processing 피드백의 `candidate.raw_path` 또는 `candidate.prepared_path`를 직접 확인하고,
...
4. 버튼 승인/거절과 단순 이모지는 이미 결정론적으로 반영된다. 다음 항목만 처리한다.
   ...
   - 질문은 레시피를 수정하지 말고 이미지와 실행 상세에 근거한 답변을 resolution에 그대로 쓴다.
...
8. 처리한 피드백마다
   `python3 Tools/ArtPipeline/art_runner.py resolve-feedback ID "사용자에게 보여줄 실제 답변 또는 처리 내용"`을
   실행한다. 이 문자열은 Slack 스레드에 그대로 게시되므로 "답변함" 같은 내부 메모가 아니라
   사용자가 바로 이해할 문장으로 쓴다.
   모호하거나 충돌하는 피드백은 해결 처리하지 말고 그대로 남긴다.
9. 피드백이 `needs_input` 상태의 게임 반영 요청에 대한 답이면 선택 내용을 intent로 요약해
