...
- **장비**: 무기 1 + 보조 1 슬롯. **어떤 장비도 공격력을 올리지 않는다** — 사거리 2(빔 랜스),
  ...
  - **원거리는 장비가 연다(M4, 2026-07-31)** — 무료 원거리를 폐지하고 `아크 캐스터`
    (무기 슬롯, 사거리 5, 제작 145G) + `에너지 셀`(칸당 4충전, 사격 1회 = 1충전,
    조합 `코어 파편 + 뇌관 화약`)로 갈랐다. 장비·탄약·사선 판정과 셀 소비는
    `RangedWeaponRules.TryFire` 하나가 원자적으로 하며 **명중 뒤에만 소비**한다
    (빗나간 사격·접근 이동은 탄을 안 먹는다). 액션 휠 원거리 토글은 무기 보유 시에만
    활성이고 남은 셀을 보여 준다. 모든 피해 경로가 `MonsterBrain.OnDamaged`를 불러
    지각 밖 저격도 추격을 유발한다(도주 중 개체는 예외). 투척 볼트는 이 게이트와 무관하다.
    수치는 실플레이 전 임시 — SSOT는 `docs/SYSTEMS.md` 「원거리 사격」.
