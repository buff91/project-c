...
- **아트 방향(전환 중)**: **v0.3.3(2026-07-30) 사이버펑크 피벗 확정** — 테마가
  ...
  **폐 아케이드 복합타워**로 재설정됐다(GDD §10.1 — 구조·생성 규칙·골든 지문 불변, 정체성만 교체).
  ...
  리스킨 표 §4-b)이 코드에 반영됐고 전체 회귀 그린. **병원 드레싱 에셋(hospital* 슬롯)만 구판**이며
  교체는 환경 네온 전환(M5) 재발주가 소유한다 — **정의·잔여 범위의 SSOT는
  `docs/ROADMAP.md` 「아트」의 「환경 네온 전환(M5)」 항목이다.**
  - **M5 1차 슬라이스 반영됨(2026-07-30)** — 공용 환경 6셀(바닥·좌우 벽·문 2종·계단)이
    ...
    `rider-ingame-live-v1.png`. 잔여: hospital* 드레싱 9슬롯·계단 특수 소스·소품 재발주.
...
- **배고픔/중간 생환**: `HungerRules`가 포만→배고픔(경고)→굶주림(주기적 HP 감소)을 소유한다.
  ...
  `ExtractionDepths`. 폐 아케이드 복합타워에서는 2F·6F다) 두 곳뿐이고 최심층은 보스를 잡아야
...
- **장비**: 무기 1 + 보조 1 슬롯. **어떤 장비도 공격력을 올리지 않는다** — 사거리 2(빔 랜스),
  명중 넉백(임팩트 렌치), 피해 -1(전광판 방패), 안전 낙하 +2(서스펜션 부츠)처럼 규칙만 바꾼다.
...
- **최근 검증 기준** — **숫자에는 반드시 기준 커밋 해시를 함께 적는다.** 해시 없는 개수는
  ...
  - Unity PlayMode `ProjectC.Tests.PlayMode` **2/2 통과** (`9d6a37e` + 미커밋 작업 트리,
    같은 세션, 3.2s) —
    ① 폐 아케이드 복합타워 B2 → 8F 보스 → 출구(치트 훅과 SPACE 경로 양쪽) ② 침수된 금고 구역 1 → 보스 없는 최심부 출구.
