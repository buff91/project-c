...
- **저장 내구성**: 런 체크포인트와 메타 창고는 `JsonFileStore`를 공유한다. JSON은 같은 디렉터리의
  `.tmp`에 먼저 flush한 뒤 최종 파일로 교체하고, 직전 정상본은 `.bak`으로 보존한다. 기본 파일이
  없거나 손상되면 백업을 읽어 기본 파일까지 복구한다. 런 종료/포기 때는 기본·백업·임시 파일을
  함께 지워 이미 끝난 판이 되살아나지 않는다.
