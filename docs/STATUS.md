...
- **UI/해상도**: 화면공간 UI는 UI Toolkit. `MainMenuHUD`, `HubHUD`,
  ...
  **논리 캔버스는 640×360이다** — `PanelSettings`가 `ConstantPixelSize`이고 배율은 `UiPanelScale`이
  ...
  15px 클릭 영역으로 넓혀 자유 카메라를 플레이어에게 되돌린다. 평상시는 `B2 · 현재 플레이 층`, 수직
  관찰 때만 `관찰 B1 ▲/▼ · 이동 불가`를 쓴다. 지도 배지와 플레이어 마커는 계속 실제 플레이 층을
  가리킨다. 좌표는 디버그 패널에만 둔다. 행동 피드백은 3초 뒤 강조만 빠지고
  ...
  **PC 월드 카메라의 기본 배율은 허브·던전 모두 `playCameraSize` 2.3이다.** 허브는 13×9 캠프를
  ...
  B2 시작 hero-room도 예외 없이 2.3을 쓴다. 이전 HUD-safe fit(PC 실측 2.823529)은 방 안쪽 셀에만
  적용돼 문을 넘을 때 월드가 축소·확대됐으므로 제거했다. 이제 방 안·문·복도와 q0..q3가 모두 같은
  배율이고 플레이어 위치·활성 층·턴도 불변이다. 불규칙한 위/아래층 관찰 패치의 임시 카메라만
  네 시점 최대 필요 배율을 공유하므로 관찰 중 Q/E에서도 크기가 뛰지 않는다. 실제 문 안/밖 2.3 비교본은
  `docs/captures/camera-door-{inside,outside}-pc-2026-08-04.png`다.
...
- **던전 화면 톤 / 메인 원정자**: PC Game View는 청흑 void·불투명 panel 안개 위에
  ...
  방향 클립이 있을 때 자동으로 꺼진다. 정적 폴백도 방향에 따라 몸체 배율을 바꾸지 않고 좌우 반전과
  후면 오프셋만 사용한다. 층 전환은 몸체·
...
- **FOV/전투 정보**: Unknown/Explored/Visible 3상태. `MappedSilhouette`는 네 번째 FOV 상태가 아니라
  현재 활성 층의 별도 지도 지식이다. Unknown인 일반 방·복도·문 윤곽도 `Floor`/`Barrier`/`Door`/`Gap`
  공용 저대비 범주로 표시하고, 범주는 지도 구성/비밀방 공개 시 기억해 시야 밖 타일 상태 변화로 갱신하지 않으며
  클릭해 자동 이동할 수 있지만, 실제 타일 종류·재질·원소 상태·적·아이템·프롭은 숨긴다. 미공개
  비밀문 좌표와 비밀방 footprint는 조사/폭발 공개 전까지 mapped 집합에서 빠져 평범한 외곽 경계에 묻힌다.
  층 전환 계단은 공용 바닥처럼 보이는 미확인 상태뿐 아니라 확인된 뒤에도 다른 mapped 목적지 자동 경로의
  중간 노드로 밟지 않고, 계단 자체의 명시적 탭만 층을 전환한다.
  ...
  요구한다. 이미 적이 보이면 일반 바닥·mapped 목적지뿐 아니라 문·공격·구출·휴식·탈출·프롭 등
  모든 자동 접근이 탭당 한 행동만 수행한다. 즉 **이동 1칸 또는 이미 인접한 행동 1회**이며, 이동 뒤
  공격·상호작용을 같은 입력에서 연달아 실행하지 않는다. 안전하게 접근하던 마지막 칸에서 새 적·피해·
  새 아이템이 생겨도 후속 행동 전에 멈춘다. 인접 구출도 성공하면 적 페이즈까지 진행하는 정식 1행동이다.
  재탭 취소는 다음 행동 경계에서 적용하되 목적지 도착이
  우선한다. PC 하단 턴 칩은 이를 `자동 이동`/`위협 · 1행동`으로 상시 설명하고, 수직 관찰 중에는
  `관찰`을 우선한다. 두 런타임 상태는
  `docs/captures/travel-mode-{safe,threat}-pc-2026-08-04.png`에서 확인했다. 비활성 층은 기존 Hole 국소
  미리보기 외에 mapped 표시·입력이 없다. 2026-08-04 후속 판독성 패스에서 같은 층의
  mapped Unknown은 실제 바닥·벽 면 재사용을 끊고 **중립 스틸 점묘 + 끊긴 다이아 + 범주 기호**로 바꿨다.
  다른 층 관찰은 실제 재질이 남은 연속 아이스 면·개구부당 한 번의 층 라벨이므로 이제 색뿐 아니라 형태
  문법으로도 갈린다. PC QHD 근거는 `docs/captures/layer-grammar-current-pc-2026-08-04.png`와
  `docs/captures/layer-grammar-vertical-compare-pc-2026-08-04.png`다.
  시야 밖 적의 피해·사망 UI는
  ...
  **개구부 너머 미리보기도 진짜 FOV다** — 반대편 층에서 `GridVisibility.Compute`를 한 번 더
  ...
  잠긴다. 명시적 관찰 조각은 passive 조각보다 더 벌리고 밝은 시안 틴트로 구분한다. 위·아래층 모두
  포인터 타일에 X형 읽기 전용 마커를 띄우며, 클릭하면 같은 자리에서 마젠타로 응답하고 `VIEW ONLY`와
  이동 불가 로그를 남기되 턴·플레이어 위치·활성 층은 그대로다. HUD는 `관찰 전용 · 직접 이동 불가`를
  지속 표시하고 잠긴 행동 버튼도 비활성화한다. 층간 투척 조준 중에는 X 대신 실제 유효 칸 마커가 우선한다.
  focus는 passive FOV의 교집합만 밝히므로 벽 뒤 정보는 추가로 열리지 않는다. PC 실화면은
  `docs/captures/vertical-observe-{down,up}-pc-2026-08-04.png`다.
  ...
  **폐 아케이드 복합타워**로 재설정됐다(GDD §10.1 — 구조·생성 규칙·골든 지문 불변, 정체성만 교체).
  ...
  - **B2 시작방 배치 정합(2026-08-01)** — 폭발통은 단순 장식이 아니라 밀기·낙하·화염
    ...
    - **B2 방 단위 바닥 조명 v1(2026-08-02)** — 매크로 밖에 남은 큰 회색/갈색 체크무늬는
      ...
      이동/공격 오버레이·Core 조명 규칙은 불변이다. 4시점 실화면은
      `docs/captures/b2-floor-light-coherence-q{0,1,2,3}-live-v1.png`다. 2026-08-04에는 같은 보정을
      활성 층의 보이는 `Floor` 4방향 연결 영역 전체로 일반화했다. Door·Stairs가 영역을 분리하므로
      방 사이 광량은 섞이지 않고, 6F에서 되살아난 타일별 명암 체크무늬도 같은 규칙으로 제거됐다.
    ...
    - **B2 배경 프롭 품질 수직 슬라이스 v2(2026-08-02)** — 승인 q0 방향판을 입력으로 만든
      ...
      비기능 저채도 설비 패널만 하나씩 둔다. 당시 방+가장 가까운 실제 문을 HUD 안전영역에 맞추던
      4시점 auto-fit 캡처는 아트 배치 근거로만 남긴다. 현행 일반 PLAY 카메라는 B2 안/밖 모두 고정 2.3이고
      수직 보기만 별도 구도를 쓴다. 정식 원본 64/64 검증과 당시 PC 4시점
...
- **던전 공통 톤**: 모든 깊이는 `project-c-torchstone.gpl` 마스터 팔레트와
  ...
  기본 환경은 청흑 void와 저채도 쿨 차콜·건메탈 바닥, 물리 광원은 제한된 앰버,
  출구·시설 신호는 틸로 읽히게 한다. 녹·갈색은 손상 가장자리와 실제 광원 반응에만 제한한다.
  ...
  **밴드 바닥 6종이 도착했다(2026-07-30, 2026-08-04 반복 안전 재마감)** —
  `env-floor-{mid,deep,boss}(-raised)`가 카탈로그 밴드 슬롯 6개에 연결됐고, 절차 오버레이
  (`BandOverlayColor`)는 슬롯 누락 때만 쓰는 폴백으로 남는다. 생성 시트의 웜 색·불규칙 알파·긴 균열은
  이제 최종 픽셀이 아니라 2×2 디테일 위치 힌트만 소유한다. conform은 공용 4,098px canonical
  다이아/외곽 3px와 `grey-2/3/4`만 허용하고, 공용 픽셀 비율을 mid 90% / deep 84% / boss 78%로
  고정한다. flat/raised top은 동일하며 실제 립은 런타임 단차 생성이 소유한다. 전용 손상 타일은 안정
  해시의 약 10~13% 비인접 좌표에만 나오고 Facility 드레싱도 3/32로 줄어, 한 균열이 모든 셀에
  반복되던 패치워크를 막는다. PC 근거는 `docs/captures/floor-band-coherence-6f-pc-2026-08-04.png`다.
