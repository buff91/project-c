...
- **지역(원정지) 정체성**: 콘텐츠 변주 표가 **(지역 × 깊이)** 두 축이다 —
  ...
  - **지역은 혼합·밀도·무대 확률을 가른다.** 일반 아키타입 스탯·행동 트리는 전 지역 공용이지만,
    Flooded 중반부터는 지역 전용 원거리 적 `ArcDrone`이 합류한다. 명중점 주변과 이어진 웅덩이를
    `ShockRules`로 통전시키며, 같은 물 위의 다른 적도 피해를 받아 역이용할 수 있다.
  ...
  - **아직 안 열렸다**: `Flooded`/`Ember` 던전은 `isAvailable: false`라 지역 전용 적을 포함한
    이 값들은 실플레이 미검증이다. 기름 타일 무대도 없다(기름은 아이템뿐).
    다음은 침수된 금고 개방과 합선 드론 시각 검증.
