...
- **최근 검증 기준** — **숫자에는 반드시 기준 커밋 해시를 함께 적는다.** 해시 없는 개수는
  ...
  - Unity EditMode `ProjectC.Tests.EditMode` — **재측정 필요**(이 환경에 에디터가 없다).
    직전 기준선은 **1043/1043 @ `42266cc`**이며 그 뒤로 EditMode 테스트가 더 붙었다.
  - Unity PlayMode `ProjectC.Tests.PlayMode` — `FirstDungeonSmokeTests`의 `[UnityTest]` **2개**
    (① 폐병원 B2 → 8F 보스 → 출구, 치트 훅과 SPACE 경로 양쪽 ② 침수된 금고 구역 1 → 보스 없는
    최심부 출구). 개수는 소스에서 센 값이고 **통과 여부는 재측정 필요**다.
  - 그 앞 기준선은 개구부 확장 `3744f04` 시점의 Core 881 / EditMode 1005였다.
