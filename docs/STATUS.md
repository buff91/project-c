...
- **UI/해상도**: 화면공간 UI는 UI Toolkit. `MainMenuHUD`, `HubHUD`,
  ...
  PC 던전 HUD는 상단 계기·미니맵과 하단 도크를 한 단계 줄였고, 퀵슬롯은 아이콘+수량만 남긴다.
  좌표는 디버그 패널에만 두고 상시 HUD에는 높이만 표시한다. 행동 피드백은 3초 뒤 사라지며,
  수직 이동 안내는 1회성 발견 카드 + 실제 장치 위에 섰을 때의 문맥 한 줄만 남긴다.
