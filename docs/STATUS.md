...
- **UI/해상도**: 화면공간 UI는 UI Toolkit. `MainMenuHUD`, `HubHUD`,
  ...
  **PC 월드 카메라의 기본 배율은 허브·던전 모두 `playCameraSize` 2.3이다.** 허브는 13×9 캠프를
  ...
  B2 시작 hero-room의 HUD-safe fit만 예외지만, 네 회전 투영의 필요 배율 중 최댓값 하나를 공용으로 써
  Q/E가 중심만 바꾸고 화면 크기는 바꾸지 않는다. 실제 PC q0..q3는 모두 2.823529였고 회전 동안
  플레이어 위치·활성 층·턴도 불변이었다. 불규칙한 위/아래층 관찰 패치의 임시 카메라도 같은 방식으로
  네 시점 최대 필요 배율을 공유하므로 관찰 중 Q/E에서 크기가 뛰지 않는다.
...
- **FOV/전투 정보**: Unknown/Explored/Visible 3상태. `MappedSilhouette`는 네 번째 FOV 상태가 아니라
  ...
  요구한다. 이미 적이 보이면 탭당 한 행동만 수행하고 재탭 취소는 다음 행동 경계에서 적용하되 목적지 도착이
  우선한다. 비활성 층은 기존 Hole 국소
