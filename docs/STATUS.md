...
- **수직 이동 의미**:
  ...
  - `Ladder`: **계단과 다른 것이다** — 한 번에 여러 단을 오르고, **명시적 링크로만** 통과한다
    ...
    **몬스터는 `MonsterArchetype.CanClimb`가 true인 종만 오른다**(기본값 false — 새 아키타입이
    ...
    배우지 않아도 읽힌다. 그래서 캐치워크가 드론에 대한 피난처가 된다. 층 전환 계단 링크는 어느 쪽 끝도 사다리가
...
- **투척 조준**: 폭탄·냉기 폭탄·기름 병은 조준에 들어가면 현재 FOV 안에서 실제로 투척 가능한
  타일을 아이템 색의 낮은 알파 바닥 마커로 표시한다. 투척 단검은 사거리·시야선이 성립하는
...
- **지역(원정지) 정체성**: 콘텐츠 변주 표가 **(지역 × 깊이)** 두 축이다 —
  ...
  - **월드 아이템 드롭 편성은 `DungeonLootRules`가 지역별로 가른다** — 다만 **모든 지역이 같은
    ...
    `DungeonGenerator.Placement`). 침수된 금고는 기름 병을 빼고 그 자리를 냉기 도구에 넘긴다 —
  ...
  - **침수된 금고 개방**: `Flooded`는 `isAvailable: true`이며 허브 선택 → 10개 구역 →
    보스 없는 최심부 출구 완주 경로를 PlayMode 스모크로 고정했다. 중반부터 `ArcDrone`과
...
- **던전 공통 톤**: 모든 깊이는 `project-c-torchstone.gpl` 마스터 팔레트와
  `ProjectCEnvironmentCatalog`의 런타임 역할색을 공유한다. 팔레트는 **코어 18색(인덱스 0~17)이
  재배치 금지**이고(런타임 카탈로그·테스트가 그 순서를 단언한다) 그 뒤에 포스트아포 재료색(18~)과
  스프라이트 셰이딩 전용 재료 계조(30~)가 덧붙는 구조다 — 색 수의 SSOT는 `.gpl` 파일이다.
  기본 환경은 청흑 void와
...
- **배고픔/중간 생환**: `HungerRules`가 포만→배고픔(경고)→굶주림(주기적 HP 감소)을 소유한다.
  ...
  `ExtractionDepths`. 폐병원에서는 2F·6F다) 두 곳뿐이고 최심층은 보스를 잡아야
...
- **최근 검증 기준** — **숫자에는 반드시 기준 커밋 해시를 함께 적는다.** 해시 없는 개수는
  ...
  - Core shim `./Tools/CoreTests/run-core-tests.sh` **1027/1027 통과** (`9e21b1c` + 미커밋 작업 트리,
    2026-07-29 실측).
  - ArtPipeline Python 회귀 **106/106 통과** (`fa6c90e` + 미커밋 작업 트리, 2026-07-28 실측).
  - Unity EditMode `ProjectC.Tests.EditMode` **1191/1191 통과** (`9e21b1c` + 미커밋 작업 트리,
    2026-07-29 에디터 실측, 28.4s).
  - Unity PlayMode `ProjectC.Tests.PlayMode` **2/2 통과** (`9e21b1c` + 미커밋 작업 트리,
    같은 세션, 2.3s) —
  ...
  - **128-레짐 전환(`4ab4438`) 이후 첫 에디터 회귀다.** 임포터 PPU를 64→128로 바꾸고 자산을 전량
    ...
    **에러 0**으로 넘어갔다. 그 전까지 세 커밋(`4ab4438`·`581445a`·`fa6ac29`)이 "에디터 세션에서
    확인 필요"를 남긴 채 shim만 돌고 있었다.
...
- **모달 렌더 검증 (2026-07-29 에디터 세션, `9e21b1c` + 작업 트리)** — 게임 메뉴·출구·결과 모달은
  **닫힘이 기본값이라 여태 한 번도 캡처된 적이 없었다.** 열어 놓고 찍자 결함 둘이 바로 나왔다.
  ...
  - ⚠ **미해결 — 출정 준비 모달이 세로로 넘친다.** 640×360 캔버스에서 상세 패널(y 557)과
    이동 버튼(y 562·594)이 **화면 밖**이라 버튼으로는 옮길 수 없다(드래그·탭 드롭만 동작).
    캔버스가 960×540 → 640×360으로 줄어든 뒤 이 모달을 재검증하지 않은 결과로 보인다.
    `docs/captures/loadout-unit-transfer-2560x1440.png`가 그 상태다.
