...
Primary request: Repaint the entire current game screen into one cohesive post-apocalyptic anomalous-labyrinth visual target. Preserve the isometric camera, the player-centered room, the functional HUD zones (health at upper left, floor/minimap tools at upper right, status near the room, action dock along the bottom), and the basic navigable room geometry. Redesign the art, not the game rules.
Scene/backdrop: a collapsed underground transit maintenance room after civilization, with cracked concrete slabs, broken ceramic tile, exposed rebar and cables, rusted pipes, a damaged blast door, sparse weeds or damp growth, and a visible lower-level breach. One restrained teal anomaly signal contrasts with practical amber emergency lights.
...
Characters: readable practical scavenger silhouettes with post-apocalyptic clothing and equipment, no knight armor, no fantasy weapons; player and enemy must separate clearly from the floor at gameplay scale.
...
Lighting/mood: cold blue-black ambient darkness with localized amber pools that reveal material texture; teal only for anomaly/system signals; retain readable midtones instead of crushing everything to black.
