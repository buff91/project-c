...
- 현재 화면공간 UI의 첫 상태 전이는 `vertical-route-discovery`다. `is-open`이 `opacity`와
  `translate`를 0.16초 동안 바꾸고, 컨트롤러는 클래스만 토글한다. 다음 카드로 넘어가기 전에 한 프레임을
  비워 닫힘→열림 경계가 같은 스타일 패스로 합쳐지지 않게 한다. **DOTween 호출부와 컨트롤러 보간은
  여전히 0곳**이다 — 이 전환은 위 표의 USS 소유 사례다.
...
PC Field Deck에서는 glyph만 네이티브 12×12로 바꾸고 셀 hit area·링 반경은 유지한다.
`HudWheelPlacement`가 여섯 셀의 전체 외곽을 화면 안으로 제한한 뒤, 현재 보이는 바이탈·우상 계기·
메시지 로그·수직 힌트·하단 레일·보스/발견 패널과 겹치지 않는 가장 가까운 중심을 고른다.
이 계산은 레이아웃을 모르는 순수 헬퍼다. 컨트롤러는 화면 앵커와 각 VisualElement의 `worldBound`를
`action-wheel` 부모 로컬 좌표로 바꾸고, 부모 `localBound`를 화면 경계로 넘긴다. 그래서 safe-area가 HUD 루트를
이동·축소해도 패널 좌표를 로컬 `left/top`으로 잘못 쓰는 이중 inset이 생기지 않는다. 보스/발견은 transition의
첫 프레임에도 `is-open` 논리 상태로 즉시 예약한다. `display:none`에서 처음 열려 아직 `worldBound`가 0인
로그·힌트·보스는 액션 휠을 한 레이아웃 패스만 숨기고 다음 프레임의 실제 footprint로 배치한다.
M4의 오브젝트별 문맥 팝업을 구현할 때는 동일한 데이터 계약과 육각 셀 시각 슬롯을
UGUI View로 옮긴다.
화면 고정 하단 `interact-button`은 키보드/포인터용 현재 행동 단축 도크로 유지한다.

### Field Deck PC 구현 경계 (2026-08-03 작업 트리)

- 공용 `PrototypeHUD.uxml`의 요소 이름·데이터 바인딩은 유지하고, Desktop USS가 640×360에서
  바이탈·우상 단일 계기·좌하 로그·하단 두 레일을 재배치한다. Mobile wrapper에는 이 배치를 이식하지 않는다.
- `IsoPrototype.unity`와 `IsoPrototypeSceneBuilder`가 Mobile/Desktop wrapper를 각각
  `PrototypeHudController`에 연결한다. wrapper가 비어 공용 fallback을 렌더하던 기준선 오류의 코드 경로는
  닫혔다. PC QHD 정상/발견+휠/실제 보스+휠은
  `docs/captures/ui-field-deck-pc-qhd-2026-08-03.png`와
  `docs/captures/ui-field-deck-{notice-wheel,boss-wheel}-pc-qhd-2026-08-03.png`에서 확정했다.
- `ui-field-*.png` 9종은 표시 크기와 같은 12×12 hard-alpha glyph다. 의미·레이블은 공용 UXML,
...
- 위 Field Deck 기준선은 2026-08-03 전체 Unity 회귀와 PC QHD 3상태 렌더를 통과했다. 후속의 기본 닫힘
  상태와 발견 카드 `×`·미니맵 층/N/마커는
  `docs/captures/project-c-followup-{ui,discovery}-pc-qhd-2026-08-03.png`에서 다시 확인했고, 같은 작업 트리의
  전체 EditMode/PlayMode 회귀와 스크립트 컴파일 오류 검사도 통과했다. 시간에 걸쳐 보이는 보행/층 전환과 적 6종의
  실제 게임 배율은 이 정적 UI 캡처의 승인 범위가 아니라 정상 진행 플레이테스트 범위다.
...
- **시작/원정지 선택**: 시작 화면은 독립 `MainMenu` 씬, 캠프 월드는 `Hub` 씬으로 분리한다.
  ...
  확인 버튼에서만 수행한다.
- **월드 기능 발견 카드**: 수직 이동 수단처럼 공간에서 처음 알아차려야 하는 기능은 상시 범례를 늘리지 않고 `VerticalRouteDiscovered` 이벤트로 1회성 비차단 카드를 띄운다. 던전 입장 카드도 같은 전환 패널을 쓴다. `HudTransientNoticeQueue`가 같은 프레임의 서로 다른 카드를 도착 순서대로 보존하고 활성/대기 중 완전 중복만 제거한다. PC는 상단 중앙, 모바일은 상단 안전 영역 아래에 두고 각 카드가 7초 뒤 닫힘 모션을 마친 다음 다음 카드를 연다. PC의 작은 `×`는 현재 카드만 수동 종료하고 대기 큐는 보존한다. 카드 본문은 계속 월드 입력을 통과시키며, `×`는 카드가 실제로 열린 동안만 picking을 받는다. 보스 패널은 같은 슬롯에서 우선하며, 열릴 때 발견 카드를 즉시 suppress해 겹침을 막고 닫힌 뒤 활성 카드의 남은 노출 시간부터 이어서 보여 준다.
...
- **PC 수직 보기 레일**: 공용 `PrototypeHUD.uxml`의 미니맵 회전 행 아래 `▲/◆/▼` 버튼을 두되 base USS는
  ...
  선택 층은 `is-selected` 버튼·`is-viewed` 층 눈금으로 표시한다. 평상시 문장은 `B2 · 현재 층`, 관찰
  중에만 `플레이 B2 · 보기 B1`처럼 두 층을 함께 적는다. 미니맵 자체는 항상 플레이 층의 북쪽 고정이며
  내부의 층 배지·`N`·별도 플레이어 마커가 층/방향/현재 위치를 맡는다. 관찰 중에는
...
- **PC 현재 층 자유 카메라**: 던전 PLAY에서 중클릭 드래그로 카메라 중심만 옮기고, 버튼을 놓아도
  ...
  기본 추종으로 돌아온다.
  수직 보기와 달리 층·FOV·턴을 바꾸거나 행동을 잠그지 않으므로 `관찰` 턴 라벨·disabled HUD 상태를
  재사용하지 않는다. 별도 카메라 상태 버튼/상태 클래스는 만들지 않고 기존 7px 플레이어 마커에만
  15px PC 클릭 영역을 둔다. 모바일 배치는 그대로 숨긴다.
- **PC 전술 지도 창**: 우상단 미니맵 옆 `MAP` 또는 `M`으로 공용 UXML의 `#tactical-map-modal`을 열되,
  Desktop USS만 `.is-open`을 표시하고 Mobile은 계속 숨긴다. 기존 `.pc-window` 크롬 안에 방문 층 레일,
  북쪽 고정 지도, `− / 배율 / + / 현재 위치` 도구와 `드래그 이동 · 휠 확대/축소 · M/ESC 닫기` 한 줄을
  둔다. 비활성 층은 `Explored` 기억만 표시하고 `기록 지도 · 이동/전투 불가`를 지속 표기한다.
  지도 열기는 조준·낙하 확인·수직 관찰·현재 층 자유 카메라를 정리한 뒤 현재 플레이 층에서 시작하며,
  열린 동안 `IsoTapInput` 월드 명령을 차단한다. Escape 우선순위는 지도 창 닫기 뒤 기존 순서로 이어진다.
...
- **씬 간 월드 배율**: 메인·허브·던전은 같은 `PrototypePanelSettings`와 현재 Screen/Game View 해상도를 공유한다. PC 허브와 일반 던전 PLAY의 기본 상태는 직교 카메라 `playCameraSize` 2.3을 그대로 쓰고 플레이어를 추종한다. 현재 층 자유 카메라는 중심만 옮겨 진입 당시 배율을 보존한다. 명시적 수직 보기만 플레이어 몸체와 관찰 조각을 함께 남기기 위해 최대 1.35배까지 일시 확장하고 복귀 시 즉시 2.3으로 돌아온다. 허브 전체 맵을 담기 위한 auto-fit은 금지한다 — 로비의 크기가 씬 전환 전후 타일·액터 배율을 바꾸면 안 된다.
...
- **PC 액션 휠 입력**: `IsoTapInput`이 `Tab` 홀드 상태를 액션으로 노출하고 HUD는 누르는 동안만 휠을 연다.
  캐릭터 클릭 고정 경로는 상시 휠을 재현하므로 두지 않는다. Ctrl/Cmd 같은 범용 modifier는 OS 단축키와
  충돌하므로 직접 읽지 않는다.
