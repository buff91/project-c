...
namespace ProjectC.Core
{
    public static partial class DungeonGenerator
    {
        ...
        /// <summary>
        /// 엘리베이터 — 탑승구와 도착 칸을 놓는다. <b>링크는 여기서 만들지 않는다</b>:
        /// 전원이 없는 동안은 그냥 멈춰 있는 설비여야 하고, 링크가 있으면 경로 탐색이
        /// 곧바로 지름길로 써 버린다(<see cref="GridPathfinder"/>는 링크를 따라간다).
        /// 전원은 보스 처치 시 Gameplay 가 <c>Connect</c> 로 넣는다.
        ///
        /// <para>
            ...
            FloorPlan landingFloor)
        {
            ...
            var target = new GridPos(chosen.x, chosen.y, landingFloor.BaseElevation);
            map.Set(chosen, TileKind.Ladder);
            map.Set(target, TileKind.Ladder);
            entranceFloor.ElevatorShaft = chosen;
            landingFloor.ElevatorLanding = target;
        }
            ...
            DungeonRegionProfile region)
        {
            ...
            if (p.HasBranch)
            {
                ...
                {
                    // 숨은 방 보상: 기본은 진행 구간에 따라 시제품 코어/코어 파편이고, 아주 가끔만 비상 송출기가
